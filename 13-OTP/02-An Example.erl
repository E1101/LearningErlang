...
-behavior(gen_server). % tells the compiler that your module is a gen_server callback module
-include("usr.hrl").
...
%% Then:
usr:start_link(). % {ok,<0.86.0>}
usr:add_usr(700000000, 0, prepay). % ok
usr:set_service(0, data, true). % ok
usr:lookup_id(0).
%%{ok,#usr{msisdn = 700000000,id = 0,status = enabled, plan = prepay,
%%services = [data]}}
usr:set_status(0, disabled). % ok
usr:service_flag(700000000,lbs). % {error,disabled}
usr:stop(). % ok
